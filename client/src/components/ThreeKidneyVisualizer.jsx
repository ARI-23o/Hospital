...
export default function ThreeKidneyVisualizer() {
  ...
  useEffect(() => {
    ...
    let onMouseDown, onMouseMove, onMouseUp, onTouchStart, onTouchMove, handleResize;
    ...
    try {
      ...
      const leftPelvisMesh = new THREE.Mesh(
      ...
      console.warn("WebGL visualizer fallback activated:", err);
    ...
    return () => {
      ...
      if (dom && onMouseDown) dom.removeEventListener("mousedown", onMouseDown);
      if (onMouseMove) window.removeEventListener("mousemove", onMouseMove);
      if (onMouseUp) window.removeEventListener("mouseup", onMouseUp);
      if (dom && onTouchStart) dom.removeEventListener("touchstart", onTouchStart);
      if (onTouchMove) window.removeEventListener("touchmove", onTouchMove);
      if (onMouseUp) window.removeEventListener("touchend", onMouseUp);
      if (
        renderer &&
        renderer.domElement &&
        currentMount &&
      ...
      <div className="relative w-full h-[260px] sm:h-[300px] my-1 cursor-grab active:cursor-grabbing flex items-center justify-center">
        {hasWebGLError ? (
          <div className="flex flex-col items-center justify-center text-center p-4 space-y-3 z-10">
            <div className="w-24 h-24 rounded-3xl bg-teal-500/20 border border-teal-400/40 flex items-center justify-center text-5xl shadow-inner animate-pulse">
              {mode === "nephrology" ? "🫘" : "🩺"}
            </div>
            <div className="space-y-0.5">
              <span className="text-xs font-bold text-teal-300">
                {mode === "nephrology"
                  ? "Renal Filtration Visualizer"
                  : "Urinary System Visualizer"}
              </span>
              <p className="text-[11px] text-slate-300 max-w-xs">
                {mode === "nephrology"
                  ? "Cortex • Medulla • Renal Artery • Renal Vein"
                  : "Bilateral Kidneys • Ureters • Bladder Dome"}
              </p>
            </div>
          </div>
        ) : (

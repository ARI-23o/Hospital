...
export default function Header({ activeTab, setActiveTab }) {
  ...
  const navItems = [
    ...
    { id: "services", label: t("nav.services", "Specialties & Services") },
    { id: "doctor", label: t("nav.doctor", "Doctor Profile") },
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100 transition-all duration-300 animate-fade-in">
          ...
          <div className="flex items-center space-x-3 truncate">
            ...
            >
              ...
              <span className="truncate">
                {t("common.hospitalAddress", "Behind LIC Office, Chandrapur, Maharashtra")}
              </span>
            </a>
            <span className="hidden md:inline text-slate-500">|</span>
            <span className="hidden md:flex items-center gap-1 text-slate-300">
              <Clock className="w-3 h-3 text-teal-400 shrink-0" />{" "}
              {t("common.opdHours", "Mon - Sat: 9:00 AM - 7:00 PM")}
            </span>
          </div>

          <div className="flex items-center space-x-2.5 sm:space-x-3 shrink-0 text-[11px]">
            {/* Language Switcher Pills */}
            <div className="flex items-center bg-slate-800/90 rounded-lg p-0.5 border border-slate-700">
              <Globe className="w-3 h-3 text-teal-400 mx-1.5 hidden xs:inline shrink-0" />
              {languages.map((l) => (
                <button
                  key={l.code}
            ...
            <a
              ...
              className="hidden sm:flex items-center gap-1 font-bold text-teal-300 hover:text-white transition"
            >
            ...
            <button
              ...
              className="hidden md:inline-flex bg-slate-800 hover:bg-slate-700 text-slate-200 text-[10px] px-2 py-0.5 rounded items-center gap-1 transition border border-slate-700"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
          className={`flex items-center justify-between transition-all duration-300 ${isScrolled ? "h-16" : "h-[72px]"}`}
        >
          {/* Clinic Logo */}
          <div
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1 leading-none">
                <span className="text-lg font-extrabold tracking-tight text-[#0F2D59]">
                  Chandrapur
                </span>
                <span className="text-lg font-extrabold tracking-tight text-[#0D9488]">
                  Kidney Care
                </span>
              </div>
              <p className="text-[10px] font-semibold text-slate-500 tracking-wider uppercase mt-0.5">
                {lang === "mr" ? t("common.taglineMarathi") : t("common.taglineEnglish")}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <span>{t("common.bookAppointment", "Book Appointment")}</span>
            </button>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 top-[100px] z-40 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white border-b border-slate-200 shadow-2xl p-4 sm:p-6 max-h-[82vh] overflow-y-auto custom-scrollbar space-y-4">
            {/* Mobile Language Switcher */}
            <div className="bg-slate-100 p-2 rounded-xl flex items-center justify-between">
            ...
            <div className="grid grid-cols-1 gap-1">
              {navItems.map((item) => (
                ...
                >
                  ...
                  <ChevronRight
                    className={`w-4 h-4 ${activeTab === item.id ? "text-teal-300" : "text-slate-400"}`}
                  />
                </button>
              ))}
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 space-y-2">
              ...
              >
                <Calendar className="w-4 h-4" /> {t("common.bookAppointment", "Book Appointment")}
              </button>
              ...
              >
                <UserCheck className="w-4 h-4 text-teal-600" /> Dr. Portal
              </button>
            </div>
          </div>
        </div>

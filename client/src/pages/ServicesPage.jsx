...
export default function ServicesPage({ setActiveTab }) {
  ...
  const services = [
    // Nephrology
    {
      ...
      tag: 'Nephrology',
      details: 'Comprehensive staging (Stages 1 to 5), personalized nephro-protective therapies (SGLT2i/RAAS blockade), dietary renal planning, blood pressure and proteinuria management to preserve native kidney function as long as possible.',
    ...
    },
    {
      ...
      tag: 'Nephrology',
      details: 'Specialized vascular access (AV fistula/graft/catheter) planning, hemodialysis prescription optimization, adequacy monitoring (Kt/V), and continuous guidance for patients and families.',
    ...
    },
    {
      ...
      tag: 'Nephrology',
      details: 'Investigation of secondary causes of hypertension, renal artery Doppler evaluation, 24-hour ambulatory BP tracking, and tailored multi-drug regimens for resistant hypertension.',
      symptoms: ['Morning headaches', 'Blurred vision or dizziness', 'Uncontrolled BP on 3+ medications', 'Elevated home BP logs'],
    ...
    },
    {
      ...
      symptoms: ['Foamy or bubbly urine', 'Severe facial puffiness upon waking', 'Rapid weight gain from fluid', 'High cholesterol with low albumin'],
    ...
    },
    {
      ...
      tagline: 'Proactive screening for diabetics, hypertensive patients, and high-risk individuals.',
      icon: ShieldCheck,
      color: 'bg-teal-50 text-teal-700',
      tag: 'Nephrology',
      details: 'Annual screening packages for diabetic individuals, elderly family members, and those with a family history of renal impairment. Medication safety audits to avoid nephrotoxicity.',
      symptoms: ['Often silent in early stages', 'Recommended for all diabetics >1 yr', 'Family history of kidney disease', 'Regular painkiller users'],
    ...
    // Urology & Urinary Care
    {
      ...
      details: 'Metabolic evaluation (24-hour urine stone chemistry), medical expulsive therapy, recurrence prevention diets, and clinical consultation for stone-induced renal impairment.',
    ...
    },
    {
      ...
      tag: 'Urology',
      details: 'Systematic investigation of complicated and recurrent UTIs, pyelonephritis (kidney infection), structural evaluation, and customized non-antibiotic and targeted prophylactic strategies.',
    ...
    },
    {
      ...
      tag: 'Urology',
      details: 'Clinical assessment of overactive bladder, urinary retention, benign prostatic hyperplasia (BPH) symptom evaluation, and non-invasive medical management roadmaps.',
      symptoms: ['Frequent urination at night (nocturia)', 'Weak urine stream or hesitation', 'Incomplete bladder emptying', 'Urinary urgency'],
      treatments: ['Urinary flow & post-void residual evaluation', 'Bladder retraining guidance', 'Medical pharmacotherapy (Alpha-blockers/5-ARI)', 'Specialist surgical referral when indicated']
    }
  ];

  const filtered = activeTabFilter === 'all' 
    ? services 
    : services.filter(s => s.category === activeTabFilter || s.tag.toLowerCase().includes(activeTabFilter));
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div className={`w-12 h-12 rounded-2xl ${srv.color} flex items-center justify-center shadow-xs`}>
                    ...
                    <h3 className="font-bold text-[#0F2D59] text-base sm:text-lg mb-2">{srv.title}</h3>
                          ...
                          <ul className="space-y-1">
                            {srv.symptoms.map((sym, i) => (
                              <li key={i} className="flex items-center gap-1.5 text-slate-600">
                                <span className="w-1.5 h-1.5 rounded-full bg-rose-400 shrink-0"></span> {sym}
                              </li>
                            ))}
                          </ul>
                        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-gradient-to-r from-teal-800 to-[#0F2D59] text-white rounded-3xl p-6 sm:p-10 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-center md:text-left">
            ...
            <h3 className="text-xl sm:text-2xl font-extrabold text-white">Because Your Kidneys & Urinary Health Deserve the Best.</h3>

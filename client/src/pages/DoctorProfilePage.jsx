...
import { 
  ...
  Calendar, Phone, MapPin, Quote, Star, Sparkles, UserCheck, ChevronRight,
  Stethoscope, Activity, Building, ArrowRight, User
} from 'lucide-react';
...
export default function DoctorProfilePage({ setActiveTab }) {
  ...
  const doctorsList = [
    ...
    },
    {
      ...
      languages: 'English, Marathi, Hindi',
      about: 'Visiting super-specialist surgical urologists collaborating with Chandrapur Kidney Care for comprehensive urinary tract, kidney stone surgical planning, and prostate evaluations.',
      expertise: [
    ...
    },
    {
      ...
      languages: 'English, Marathi, Hindi',
      about: 'Trained medical officers and certified dialysis technologists providing round-the-clock hemodialysis monitoring under the direct clinical supervision of Dr. Sagar Sarda.',
      expertise: [
          ...
          {filteredDoctors.map((doc) => (
            <Interactive3DCard
              ...
              className={`bg-white rounded-3xl overflow-hidden shadow-soft border flex flex-col justify-between transition-all ${
                doc.isLead ? 'border-teal-400 ring-2 ring-teal-500/20 shadow-card' : 'border-slate-100'
              }`}
            >
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100">
                    ...
                    <span className="font-bold text-teal-700 truncate block">{doc.category.toUpperCase()}</span>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {doc.expertise.slice(0, 3).map((exp, i) => (
                      <span key={i} className="text-[10px] font-medium bg-teal-50 text-teal-800 px-2 py-0.5 rounded-md border border-teal-100">
                        {exp}
                      </span>
                    ))}
                  </div>
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl border border-slate-100 animate-scaleUp my-auto space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-teal-700 font-semibold">{selectedDoctorModal.designation}</p>
              </div>
              <button
            ...
            <div className="space-y-2 pt-2">
              <span className="text-xs font-bold text-slate-800 block">Areas of Specialization:</span>
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1">
              <p><strong className="text-slate-700">OPD Timings:</strong> {selectedDoctorModal.opdTimings}</p>
              <p><strong className="text-slate-700">Languages:</strong> {selectedDoctorModal.languages}</p>
            </div>
